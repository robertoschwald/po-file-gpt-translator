...
@Service
class TranslationService(val httpRequestService: HttpRequestService) {
    private val log: Logger = LoggerFactory.getLogger(this::class.java)

    private val properties = loadExternalProperties()
    private val apiKey = properties.getProperty("openai.api.key")
    private val endpointUrl = properties.getProperty("openai.api.endpoint")
    ...
    private fun translateText(text: String, targetLanguage: String): String {
        ...
        val request = Request.Builder()
            ...
            .post(requestBody)
            .build()
        val response = httpRequestService.performRequest(request) // Call on Bean Proxy for @Retryable to work
        val jsonResponse = JSONObject(response.body!!.string())
        val translatedText =
            jsonResponse.getJSONArray("choices").getJSONObject(0).getJSONObject("message").getString("content")
        log.info("Translated text >{}< to: >{}<: >{}<", text, targetLanguage, translatedText)
    ...
    private fun generateMoFile(messages: List<Message>): String {
        ...
        log.info("Translation finished. Generated .po file with {} messages", messages.size)
        val content = outputStream.toString(Charset.forName("UTF-8"))
        log.info("Generated .po file content: \n{}", content)
        return content
    }
